...
export function BaseCustomTable({ title, fields, data, actions }: BaseCustomTableProps) {
    const fieldWithPrimaryKey = fields.filter(f => f.isVisible)[0]
    return (
        <Card className="overflow-hidden">
            ...
            <CardContent className="p-6">
                <div className="mb-6">
                    {actions?.add && <AddButton href={actions.add.url} />}
                </div>
                    ...
                    <TableHeader className="text-lg">
                        <TableRow>
                            {fields.map((field, index) => (
                                field.isVisible && <TableHead key={index}>{field.columnName}</TableHead>
                            ))}
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {data.map((item) => (
                            <TableRow key={item[fieldWithPrimaryKey.columnKey]}>
                                {fields.map((field, index) =>
                                    (field.isVisible && <TableCell key={index}>{getNestedValue(item, field.columnKey)}</TableCell>)
                                )}

                                <TableCell>
                                    <div className="flex space-x-2">
                                        {actions?.edit && <EditButton href={`${actions.edit.url}${item.id}`} />}

                                        {actions?.delete &&
                                            (
                                                <TableDeleteButton canDelete={false} data={item} onDelete={actions.delete.action}></TableDeleteButton>
                                            )
                                        }
                                    </div>
                                </TableCell>
